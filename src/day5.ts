...
class Procedure {
    public static from(s: string[]) {
        const values = s
            ...
            .map(line =>
                line
                    .filter(word => !["move", "from", "to"].includes(word))
                    .map(Number)
            )

        return values.map(
            ([amount, from, to]) => new Procedure(amount, from, to)
        )
    }
    constructor(
...
;(async () => {
    ...
    const parsedProcedures = lines[1]
        .split("\n")
    ...
    const topCrates = crates
        .map(crate => crate.items[crate.items.length - 1])
        .join("")
    const topCrates2 = crates2
        .map(crate => crate.items[crate.items.length - 1])
        .join("")
